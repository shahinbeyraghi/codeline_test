...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		...
		<?php elseif (of_get_option('blog_settings') == 2) :?>
			<?php the_excerpt(); ?>
		<?php endif; ?>
        <?php echo '<p>Release Date: ' . get_post_custom_values('release_date')[0] . '</p>'; ?>
        <?php echo '<p>Ticket Price: ' . get_post_custom_values('ticket_price')[0] . '</p>'; ?>
        ...
        <?php
			foreach (get_taxonomies() as $taxonomy)
                echo get_the_term_list( $post->ID, $taxonomy, '<p>' . $taxonomy. ': ', ', ', '</p>' );
		?>
	</div><!-- .entry-content -->
	<?php endif; ?>
